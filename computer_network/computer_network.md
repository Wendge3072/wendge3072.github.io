...
家庭内部：无线设备连接到无线接入点（wireless access point），ap和有线以太网设备一起连接到路由器，路由器连接到cable modem或DSL modem，modem再连接到电信运营商的CO或有线电视运营商的headend。ap和路由器通常包含在一起。
...
发送窗口：当有新的数据包从上层到来时，被赋予窗口中下一个最高序号，窗口上边界前移一格；当有一个数据包的确认到达，窗口下边界前移一格。
...
边界网关协议：BGP，在AS之间传输数据的协议，小的AS要在大的AS购买中转服务，得到中转转发功能，小AS是大AS的客户，小AS之间不需要购买，他们是对等的关系，可以免费传输数据。
...
关闭后会等待一段时间，包生存周期的两倍，防止确认丢失。

滑动窗口（流量控制）：发送方可以窗口探测，明确确认号、窗口大小。

- 延迟确认：将确认和窗口更新时间最多延迟500ms，获得一些数据免费搭载确认段或窗口更新段。
...
- 延迟确认：将ACK与调整窗口延迟，与回传数据段一起回传。

- 发送方若发送多个小数据包，仍然低效，使用Nagle算法：发送端立刻发送开销太大。只发送第一次到达的字节，其余的缓冲直到前面的被确认，或者填满一个最大数据段。
- Clark：解决低能窗口综合症，大块数据发送，接收方交互式应用每次只读一个字节。接收方缓冲区占满，处理一个字节，缓冲区空出一位，告诉发送方发一字节，一直如此循环，则造成带宽浪费。解决方法：阻止接收方发送窗口大小为1的窗口更新，让接收方等待，直到窗口大小达到一定大小，即当接收方能够处理连接建立时宣告的最大段长度，或者缓冲区一半为空；换言之，接收端具有一半空缓存或最大段长这么大的空缓存时，才发送窗口更新。

- Nagle解决发送端发一个字节的问题，延迟确认解决接收端确认太多的问题，Clark解决接收端读取慢、窗口更新幅度小的问题。

TCP的往返延迟标准差大，不集中；链路层的标准差小，较集中。

确认TCP重传时间：不能太小，不必要的重传，导致网络拥塞；不能太大，丢失之后延迟高，性能被影响。

动态算法确定重传时间：对于每个连接，TCP维护一个SRTT，平滑往返时间，是一个估计值。每次发送，若计时器未到期，记录往返时间R，指数更新SRTT：$SRTT=\alpha SRTT+(1-\alpha)R$，一般$\alpha=\frac{7}{8}$，指数加权移动平均。随机流量负载接近最大容量时，延迟变大，且不稳定性变高，建立对方差的平滑估计：RTTVAR，$RTTVAR=\beta RTTVAR+(1-\beta)|SRTT-R|$，$\beta$一般取3/4，重传超时值被设置为$RTO=SRTT+4\times RTTVAR$

TCP四个计时器：

- 重传计时器：上文提到

- 持续计时器：发送方维护，若窗口大小被告知为0，启动该计时器，一段时间后超时，发送方发出探寻消息，若回复还是0，重启计时器，否则发数据。解决了接收方的窗口更新消息丢失的情况。
- 保活计时器：连接空闲很长时间，保活计时器超时，促使一端看另一端是否还在，若无响应终止连接。
- 在关闭过程中连接处于TIMED WAIT的计时器，超时值是最大数据包生存期的两倍，确保连接关闭时，该连接上创建的所有数据包都消失。

TIMED WAIT指断开发起方发出ACK开始，即进入该状态。



拥塞控制：使用拥塞窗口，实际窗口取接收方窗口和拥塞窗口的最小值。

拥塞：流量突发，延迟高、丢包。

小的突发数据包，在转发最慢的链路上被拉长，其确认消息返回发送方的速度表明了最慢链路的速度，这就是想要的精准发送速率，发送方以这个速度发送数据包，这是最慢链路允许的速率，就不会在沿途上的每个路由器排队、堵塞。这个时序叫做确认时钟。

慢速启动：开始时用小的初始值（4个段）初始化拥塞窗口，对于每个在重传计时器到期之前到达的确认，发送方为拥塞窗口增加一个段，每个被确认的段被允许发送两个段，每过一个往返时间，拥塞窗口加倍。



使用慢速启动阈值，最初可以随意设置，可以到达流量窗口的大小。当遇到超时，或者拥塞窗口超过阈值，或者接收方的窗口填满，就采取行动。

遇到数据包丢失，慢速启动阈值减半，再将拥塞窗口从0重新启动。

达到阈值没有超时或丢包，就线性增长拥塞窗口，每个RTT增加一个包，实际上将最大段长度加到拥塞窗口中，对cwnd/MSS中每个确认的数据包，将cwnd加上MSS*MSS/cwnd（每次加的大小与加的次数相乘正好是最大段长MSS）。

重复确认：当数据包丢失，后续数据包到达，后续包的确认包含同样的确认号，为了防止不同的路由路径造成的失序，当重复确认达到3个就认为数据包丢失。

快速重传：收到了重复确认，认为这包已丢失，立即重传，不必等待重传时钟到时。

快速恢复：TCP有累计确认的功能，丢失数据包之后的后续数据包返回相同的确认号，有三个重复确认，则重传，之后将cwnd设为阈值的一半，线性增长。

TCP选项：选择确认，确认消息SACK字段说明已经正确接受的三个序号字段，供发送方检查。

TCP：CUBIC，使拥塞窗口是自上次拥塞之后时间的函数。



TCP三次握手，为了安全，还需要TLS的两次握手；QUIC只需要三次握手，可以保障安全。



BBR：

- 达到瓶颈带宽，传递速率不变，没达到瓶颈带宽，传递速率随着数据包增多线性增长。
- 达到带宽延时积，传播延时线性增长，到达瓶颈带宽，传播延时不变。



HTTP：

- request：第一行第一个单词：请求动作，GET，POST，HEAD方法

  请求行；消息头

- response：第一行返回状态位：（协议，状态码，状态短语）
