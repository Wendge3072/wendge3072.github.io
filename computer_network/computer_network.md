...
达到阈值没有超时或丢包，就线性增长拥塞窗口，每个RTT增加一个包，实际上将最大段长度加到拥塞窗口中，对cwnd/MSS中每个确认的数据包，将cwnd加上MSS*MSS/cwnd（每次加的大小与假的次数相乘正好是最大段长MSS）。

重复确认：当数据包丢失，后续数据包到达，后续包的确认包含同样的确认号，为了防止不同的路由路径造成的失序，当重复确认达到3个就认为数据包丢失。

快速重传：收到了重复确认，认为这包已丢失，立即重传，不必等待重传时钟到时。

快速恢复：TCP由累计确认的功能，丢失数据包之后的后续数据包返回相同的确认号，有三个重复确认，则重传，之后将cwnd设为阈值的一半，线性增长。
